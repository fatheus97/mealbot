...
1. **Fetch the queue.** `gh issue list -R fatheus97/mealbot-tickets --state open --limit 30`. Skip any labelled `needs-info`, `blocked`, or `shipped` — they're waiting on the owner or already done. **Before starting each ticket, look for ITS PR** across ALL states and branch on it — this both dedups and closes out BIG tickets the owner merged out-of-band. **A ticket's PR is ONLY a PR in `fatheus97/mealbot` itself whose head branch is exactly `ticket/<N>`, or a retry's `ticket/<N>-<k>` (step 4), and which is not cross-repository** (`isCrossRepository: false` — `--head` alone also matches a fork's branch of the same name): `gh pr list -R fatheus97/mealbot --state all --head ticket/<N> --json number,state,url,isCrossRepository,closedAt` and drop every row where `isCrossRepository` is true (via the API/MCP: keep only `head.repo.full_name == "fatheus97/mealbot"`). Repeat for `ticket/<N>-2`, `ticket/<N>-3`, … and stop at the first name with no PR left; a retry is only opened after the previous attempt was declined, so the names are contiguous. If any attempt is MERGED or OPEN, that is the ticket's PR; otherwise the last attempt is. **Ignore every other PR that mentions the ticket** — never resume, push to, merge, or act on it: the repo is public, and anyone can open a PR from a fork whose body says `Ships fatheus97/mealbot-tickets#<N>`. Only someone with push access can create a branch in this repo, so the branch is the identity; the body text is not.
   ...
   - **CLOSED-unmerged PR** → declined, unless the owner has re-queued it since. The owner re-queues by removing `blocked`, and only the ticket's event log can show that (a missing label looks the same before this routine adds it as after the owner removes it): `gh api --paginate repos/fatheus97/mealbot-tickets/issues/<N>/events --jq '.[] | select(.event == "unlabeled" and .label.name == "blocked") | .created_at'`.
     - **No such event later than the PR's `closedAt`** → declined; do NOT silently re-solve. Label the ticket `blocked` (so the declined decision persists and it isn't re-flagged every run — the owner removes `blocked` to re-queue, or closes the ticket) and note it once in the report.
     - **One later than `closedAt`** → re-queued. Proceed to triage (step 2) as the next attempt (step 4 names its branch).
...
3. **Classify risk — this sets the merge bar (step 7):**
   ...
   - **BIG / risky — never auto-merge, HOLD for the owner:** anything touching
     ...
     - **a colour / background / `opacity` change you have no browser to verify** — conditional, not blanket: it applies only when your toolset can't drive the preview (the cloud routine's can't), per the carve-out in `.claude/rules/frontend.md`. A UI diff that touches no colour at all (layout, `zIndex`, `boxSizing`, event handling) stays SMALL — the two-theme check has nothing to find in it, so don't inflate every UI ticket to BIG;
...
4. **Implement** on branch `ticket/<N>` off `main` — `git checkout -b ticket/<N> origin/main`, exactly that name, since step 1 finds the PR by it (never off another open branch — see `feedback_pr_base_branch`). A re-queued ticket uses `ticket/<N>-<k>` instead, with k one past the last attempt step 1 found (`ticket/<N>` is attempt 1, so the first retry is `ticket/<N>-2`). A declined PR's branch is never deleted, so reusing its name would need a force-push:
