...
7. **Merge bar — owner's rule is "auto-merge small, hold big":**
   ...
   - **BIG / risky (by triage OR by the diff re-check)** → do NOT merge. Leave the PR green + reviewed and **ping the owner** with the PR link and a one-paragraph plain-English summary of what it does and any risk, for their glance and merge.
