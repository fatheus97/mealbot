...
| **5c. Bulk actions / multi-select (user table)** | ⬜ | S–M | 5+ | **Owner-requested 2026-07-23.** The User Management table is one-row-at-a-time; add row **checkboxes + a select-all header** and a **selection action bar** (deactivate / reactivate / delete selected). Build over the **existing per-user endpoints** (client-side loop with per-item result collection — each call is already guarded + audited), so a partial batch reports which users succeeded/failed rather than aborting. **Respect the guards:** exclude self + demo from bulk actions client-side, and rely on the backend's per-call **last-active-admin** invariant to reject the one delete/deactivate that would orphan admins (surfaced as a per-item failure, not a batch abort). Mandatory two-theme check. |
