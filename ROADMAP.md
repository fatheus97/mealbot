...
| **In-app info hints — "i" hovers everywhere (U-9)** | 🟡 | M | — | **Owner's preferred way to explain the app (2026-07-25):** small "i"-in-a-circle affordances that reveal a one-line explanation on hover / keyboard-focus / tap — deliberately **instead of** intro tours or coach-mark popups (owner finds those annoying; loves inline hints). **Reusable primitive SHIPPED:** `frontend/src/components/InfoHint.tsx` — theme-robust (self-contained surfaces, legible in light **and** dark without leaning on adaptive colour), CLS-safe (out-of-flow bubble reserves no flow space), and accessible (labelled button, `aria-expanded`/`aria-describedby`, Escape + outside-tap dismissal). **First use:** the feedback-credit eligibility hint next to *Send feedback* (explains €1/report · up to €3/mo · needs the **monthly** plan — subscribed or trialing; annual is already discounted). 🟡 = incrementally place hints on the other explain-worthy spots as those screens are next touched — no big-bang pass. **PLANNER OPTIONS DONE 2026-08-13 (#451):** three hints on the first screen a user sees — meals-per-day (silently overridden whenever a per-day layout applies — the "Customize meal types per day" toggle OR a saved default layout in Settings, since `_resolve_layout` falls back to `user.default_day_layout` and the toggle is never seeded from it), stock-only (never adds to the shopping list; short stock yields a simpler meal), and taste preferences (a hard filter, not a suggestion; only the first 20 count). Days-to-plan and people-count deliberately got NONE: a hint that restates its label teaches the user that hints are noise. Two things that fell out and are worth knowing before the next placement: **InfoHint now CONSUMES Escape**, so it can finally go inside ModalShell/PaywallModal/CookbookModal without closing them (its old docstring barred exactly that), and a hint must sit **OUTSIDE** its `<label>` with explicit `htmlFor` — a `<button>` inside a `<label>` makes that label address two controls. **Still open: paywall terms and admin metrics.** Dietary chips already carry one (the sulphite hint in DietarySelector), and the Settings toggles are explained by always-visible hint prose under each control — so converting those to hovers would be churn, not coverage. |
...
| **Bigger leftover batches (cook once, eat 3×)** | ✅ | — | leftovers ✅ | **SHIPPED 2026-08-09.** The three limits now come from a `LeftoverPolicy` chosen by DIET, not raised globally: `ADULT_LEFTOVER_POLICY` is byte-for-byte the old behaviour (1-day lookback, no fan-in, 2 links) and `BABY_FOOD_LEFTOVER_POLICY` gets a 3-day window, fan-in up to 2 dependents, and 4 links. The prediction in this row held — it needed almost no new machinery: the invariants already permitted fan-in (L11) and `portions_for_day` already computed `1 + fan-in count`; only `plan_leftover_links` declined to emit it. ⚠️ **Two things worth knowing before touching it again.** (1) The adult policy's `max_per_source=1` is *belt and braces only* — `lookback_days=1` already makes fan-in structurally impossible, so a test asserting adult no-fan-in passes even with the cap deleted; `test_max_per_source_is_what_stops_fan_in__not_just_the_lookback` is the one that actually reaches it. (2) `max_per_source` above 2 pushes portions past 3 — fine for the prompt now (it states the multiplier as a number since this PR), but re-read the batch block before assuming a 5× batch still reads sensibly to a home cook. **Not done:** the same relaxation for other batch-cooked diets (chilli, stews) — nothing has asked for it. |
...
| **"Repeat this week" / plan templates** | ✅ | — | calendar dates ✅ | **SHIPPED 2026-08-09 (#437 backend, #439 the Repeat button on every plan card).** Verbatim copy, no LLM call — the cheaper option this row recommended. `services/plan_repeat.py` recomputes the shopping list against the CURRENT fridge/staples/`include_spices` rather than copying the old one, so a repeat buys what you actually still need. Refuses a source whose `kind != "planned"`. |
| **Waste tracking** | ✅ | — | — | **SHIPPED 2026-08-09, both surfaces (#440 fridge removal, #441 finish-plan expired review).** Opt-in via `User.waste_tracking_enabled`, **off by default and enforced server-side** — a disabled user gets `{"recorded": 0}` for a non-empty post. Capture is ungamified and skippable exactly as this row asked: the fridge question rides the removal confirm the user already gets, and the finish-plan review is machine-derived and dismissable in one click. ⚠️ **`WasteRecord` is WRITE-ONLY on purpose and carries a DELETE-BY tripwire** — the signal is a time series that cannot be backfilled, so capture ships before any reader; if nothing consumes it within a couple of months of 2026-08-09, delete the table. ⚠️ **`still_fine` rows are NOT waste** — they are the denominator, so any query that sums "waste" must filter on `reason`. The advertisable number this row wanted needs that consumer, which does not exist yet. |
...
|---|---|---|---|
| **Non-root SSH hardening** | ⬜ | S | Server-side, not in repo. Create a personal sudo user, disable root SSH login. Low urgency, easy to forget — do it at deploy time. **Was badged 🟡 with a description in which nothing has happened** — a partial badge on unstarted work reads as "someone is on it", which is how an item gets skipped in a scan. **Scope it honestly, because there are two ways to overstate this and an earlier draft of this very row hit one of them.** Most automation runs as a separate `deploy` user — 7 of the 8 units in `deploy/systemd/` carry `User=deploy`, and the CI deploy key's forced command lives in `~deploy/.ssh/authorized_keys`. But **`mealbot-unit-sync.service` has no `User=` and therefore runs as root**, deliberately and unattended on its timer, because writing `/etc/systemd/system/` and calling `daemon-reload` admit no lesser privilege (its own header says so). So root execution on this box is *not* reducible to the operator's interactive login, and a draft of this row that claimed it was got that from counting the `User=deploy` files without checking the one that had none. Second overstatement to avoid: `deploy` is in the docker group, and the docker socket is root-equivalent by construction (it can mount the host filesystem into a container), so the `deploy`-run units are not sandboxed either. **What closing root SSH actually buys is one narrowed path — the interactive one — not the removal of root from the box.** Worth doing; not worth ticking as though it were containment. |
...
| **`passwordresettoken` retention sweep** | ✅ | S | Housekeeping, **not** a launch blocker. Reset tokens are stamped `used_at` and never deleted, so the table grows monotonically. `ix_passwordresettoken_expires_at` already exists for exactly this (a global `DELETE ... WHERE expires_at < cutoff`) — the index landed ahead of the job, same as `ix_authsession_expires_at` did before `authsession_cleanup`. Simpler than that one: no self-referencing FK to sever, so it's a plain DELETE plus a systemd oneshot. Insert volume is bounded by the 60s per-account cooldown, the 5/min IP limit and the one-live-token-per-user index, so it never threatened the VPS in the meantime. **✅ SHIPPED 2026-08-08** — `services/passwordresettoken_cleanup` + `app.scripts.passwordresettoken_cleanup` + `mealbot-passwordresettoken-cleanup.{service,timer}`, daily 04:00, **30-day** retention (longer than authsession's 7 because a reset row is the audit trail for "someone changed my password", a question that reaches an operator weeks later, and the volume that buys is negligible). Two things the build turned up that the sentence above had subtly wrong: the one-live-token index is predicated on **`used_at IS NULL`, not on expiry**, so a user holds at most one UNSTAMPED row *ever*; and a second reset request does not insert alongside the first, it **supersedes by stamping it** (`void_outstanding_tokens` runs an UPDATE, not a DELETE, and it is called on the mint path at `password_reset.py:102`). So the rows this sweeps are overwhelmingly stamped ones, and the growth driver is reset *requests* rather than outstanding links. No manual step on the box — `sync-systemd-units.sh` installs and enables a new timer on its own (§7). |
...
done, as is the **`passwordresettoken` retention sweep** (**shipped 2026-08-08**);
the next-cheapest product win is **"repeat this week"** (S–M), and the cheapest
remaining risk-reducer is **cross-provider LLM fallback** — one line in
`LLM_MODELS` once a funded non-Gemini key exists, i.e. blocked on the owner, not
on engineering. The **admin-dashboard polish** (Admin & operations 4b) and the
