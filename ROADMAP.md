...
(reference layer researched; **schema+compat slice SHIPPED #283**; next:
reference-layer encoding → prompt → allergen screen → UI) and **Growth phase 2
...
schema+compat (**✅ SHIPPED #283**) → reference-layer encoding → prompt → allergen screen → UI. If it pans out, it changes what the whole Growth push is
