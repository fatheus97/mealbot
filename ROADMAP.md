...
>
> ⬜ **Still open, and a prerequisite if registration is ever opened:** there is
> no email-*change* path (`UserUpdate` has no email field), so a typo'd signup
> address is a permanent hard lockout whose only remedy — admin force-verify —
> stamps the *wrong* address as verified.
