...
| **Inline step timers — reach + robustness** | 🟡 | S–M | cooking mode ✅ | **The idea is ALREADY SHIPPED (#152, 2026-07-02)** — re-proposed by the owner 2026-07-26, so this entry records what exists and what is genuinely missing rather than duplicating it. `tokenizeStepTimers` (`frontend/src/components/recipe/cookMode.utils.ts`) splits a step into plain-text and duration segments, and `CookMode` renders each duration as a `<button>` that starts the countdown — "Simmer for **10 minutes**" → tap → 10:00 — with pause/cancel, a manual-minutes fallback, a WebAudio alarm and a `Notification`. **What's actually open:** **(1) wall-clock drift — the one real defect, not a nice-to-have.** The countdown is a `setInterval` 1-second decrement over React state, so a backgrounded tab (throttled to ~1/min) or a locked phone silently *stalls* the timer; it should derive `remaining` from a stored target timestamp instead. A cooking timer you can't put the phone down next to is broken. **(2) English-only parsing.** The regex matches `hours?\|hrs?\|minutes?\|mins?`, but `language` is a user preference and the LLM writes steps in it — a Czech/German plan's "5 minut" / "5 Minuten" never becomes a button. Also unmatched: seconds, ranges ("10–12 minutes"), spelled-out numbers, "an hour", "1½ h". **(3) One timer at a time** — state is `Timer \| null`, so starting a second replaces the first; pasta + sauce is the everyday two-timer case. **(4) Cook mode only** — `RecipeSteps` (the plain step list in `MealCard` / `CookNowForm` / `CookbookModal`) renders steps as inert text. **(5) The timer dies with the modal** (component state), so closing cook mode to check the fridge loses it. **Slicing:** (1) alone is a small, high-value fix; (3)+(5) are one state refactor (a timer list lifted above the modal); (2) is bounded by whatever the language whitelist actually allows, and shares its shape with the English-only matching in `allergen_screen.py`. |
