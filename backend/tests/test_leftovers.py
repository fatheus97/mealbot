...
class TestSchema:
    def test_ordinary_meal_has_no_link(self) -> None:
        assert meal().leftover_of is None
        assert meal().is_leftover is False

    def test_leftover_reports_is_leftover(self) -> None:
    ...
    def test_legacy_blob_without_the_key_still_parses(self) -> None:
    ...
    def test_leftover_with_ingredients_is_rejected_at_the_model(self) -> None:
    ...
    def test_ordinary_meal_may_have_empty_ingredients(self) -> None:
...
class TestLlmSchemaSplit:
    def test_generated_meal_has_no_leftover_field(self) -> None:
    ...
    def test_leftover_ref_absent_from_the_llm_json_schema(self) -> None:
    ...
    def test_llm_schema_descriptions_stay_short(self) -> None:
    ...
    def test_planned_meal_still_carries_the_field(self) -> None:
        assert "leftover_of" in PlannedMeal.model_fields

    def test_to_planned_day_widens_with_no_links(self) -> None:
        raw = LlmDayResponse(
            meals=[
                GeneratedMeal(
                    name="Soup",
    ...
    def test_to_planned_day_preserves_content(self) -> None:
        raw = LlmDayResponse(
            meals=[
                GeneratedMeal(
                    name="Stew",
...
class TestGraphInvariants:
    ...
    def test_l2_meal_out_of_range_uses_the_target_days_length(self) -> None:
    ...
    def test_l2_accepts_a_ref_valid_only_under_the_target_days_length(self) -> None:
    ...
    def test_l3_self_reference(self) -> None:
    ...
    def test_l4_forward_across_days(self) -> None:
        p = plan([meal("X", leftover_of=(1, 0))], [meal("Roast")])
        (v,) = validate_leftover_graph(p)
        assert "points forward" in v

    def test_l4_forward_same_day(self) -> None:
        p = plan([meal("X", leftover_of=(0, 1)), meal("Roast")])
        (v,) = validate_leftover_graph(p)
        assert "points forward" in v

    def test_l4_forward_reported_as_direction_not_bounds(self) -> None:
    ...
    def test_l5_leftover_of_a_leftover(self) -> None:
        p = plan(
            [meal("Roast")],
    ...
    def test_l6_cycles_are_structurally_impossible(self) -> None:
    ...
    def test_l7_source_has_no_ingredients(self) -> None:
    ...
    def test_l10_first_meal_of_the_plan_cannot_be_a_leftover(self) -> None:
    ...
    def test_l11_two_leftovers_may_share_one_source(self) -> None:
        p = plan(
            [meal("Sunday roast")],
...
class TestRepair:
    def test_repair_nulls_the_link_and_keeps_the_meal(self) -> None:
    ...
    def test_repair_leaves_valid_links_intact(self) -> None:
        p = plan(
            [meal("Roast")],
    ...
    def test_validate_without_repair_does_not_mutate(self) -> None:
        p = plan([meal()], [meal("Broken", leftover_of=(9, 9))])
...
class TestDependents:
    ...
    def test_finds_every_dependent(self) -> None:
        p = plan(
            [meal("Sunday roast")],
            [meal("Mon lunch", leftover_of=(0, 0))],
            [meal("Tue lunch", leftover_of=(0, 0))],
    ...
    def test_does_not_confuse_different_sources(self) -> None:
        p = plan(
...
class TestPlanLeftoverLinks:
    ...
    def test_no_links_for_a_single_day_plan(self) -> None:
        assert plan_leftover_links([["hot_dinner"]]) == []

    def test_links_lunch_to_the_previous_days_dinner(self) -> None:
    ...
    def test_picks_the_right_slots_within_a_day(self) -> None:
        layouts = [
    ...
    def test_unknown_slot_strings_do_not_match(self) -> None:
        assert plan_leftover_links([["not_a_real_slot"], ["light_lunch"]]) == []

    def test_never_makes_a_source_of_a_slot_already_used(self) -> None:
    ...
    def test_produces_a_graph_the_validator_accepts(self) -> None:
...
class TestPortionsForDay:
    def test_all_ones_without_links(self) -> None:
        assert portions_for_day([], 0, 3) == [1, 1, 1]

    def test_source_slot_is_doubled(self) -> None:
    ...
    def test_fan_in_triples(self) -> None:
    ...
    def test_ignores_out_of_range_source_slots(self) -> None:
...
class TestPlannerNeverChains:
    ...
    def test_main_course_only_layout_does_not_chain(self) -> None:
    ...
    def test_main_course_in_second_slot_does_not_chain(self) -> None:
    ...
    def test_main_course_layout_still_produces_a_valid_graph(self) -> None:
    ...
    def test_mixed_layouts_do_not_chain(self) -> None:
...
class TestExpandLeftoverGroups:
    ...
    def test_no_links_leaves_the_set_alone(self) -> None:
        p = plan([meal("A"), meal("B")], [meal("C"), meal("D")])
        assert expand_leftover_groups({(0, 0)}, p) == {(0, 0)}

    def test_freezing_the_source_freezes_its_leftover(self) -> None:
        p = self._linked_plan()
        assert expand_leftover_groups({(0, 0)}, p) == {(0, 0), (1, 0)}

    def test_freezing_the_leftover_freezes_its_source(self) -> None:
        p = self._linked_plan()
        assert expand_leftover_groups({(1, 0)}, p) == {(0, 0), (1, 0)}

    def test_unrelated_frozen_meals_are_untouched(self) -> None:
        p = self._linked_plan()
    ...
    def test_does_not_mutate_the_caller_set(self) -> None:
        p = self._linked_plan()
        original = {(0, 0)}
        expand_leftover_groups(original, p)
        assert original == {(0, 0)}

    def test_fan_in_pulls_in_every_dependent(self) -> None:
    ...
    def test_reaches_a_fixed_point_across_separate_groups(self) -> None:
    ...
    def test_out_of_range_link_does_not_crash(self) -> None:
...
class TestLeftoverSourceNames:
    def test_snapshots_each_leftovers_source_name(self) -> None:
    ...
    def test_skips_an_unresolvable_link(self) -> None:
        p = plan([meal("A")], [meal("B", leftover_of=(0, 0))])
