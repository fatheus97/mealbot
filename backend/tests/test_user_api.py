...
class TestRegister:
    async def test_register_success(self, unauthed_client: AsyncClient):
        with patch.object(settings, "registration_enabled", True):
            resp = await unauthed_client.post(
                "/api/users/register",
                json={"email": "new@example.com", "accept_terms": True, "password": "NewPassword123"},
            )
    ...
    async def test_register_duplicate_email(
        ...
        with patch.object(settings, "registration_enabled", True):
            resp = await unauthed_client.post(
                "/api/users/register",
                json={"email": TEST_EMAIL, "accept_terms": True, "password": "AnotherPass123"},
            )
    ...
    async def test_register_blocked_when_disabled(self, unauthed_client: AsyncClient):
        with patch.object(settings, "registration_enabled", False):
            resp = await unauthed_client.post(
                "/api/users/register",
                json={"email": "blocked@example.com", "accept_terms": True, "password": "ValidPass123"},
            )
    ...
    async def test_register_race_falls_back_to_409_not_500(
        ...
        payload = {"email": email, "password": "RacePass123", "accept_terms": True}
        with patch.object(settings, "registration_enabled", True):
...
class TestRegisterEventLogging:
    async def test_register_emits_event_without_email_plaintext(
        ...
        with (
            ...
            resp = await unauthed_client.post(
                "/api/users/register",
                json={"email": email, "accept_terms": True, "password": "ValidPass123"},
            )
        assert resp.status_code == 201
